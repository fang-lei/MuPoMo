...
# multipop.R ------------------------------------------------------------------------------ Description: Estimate and forecast
# mortality rates based on a semi-parametric approach, which applies parametric modelling for multiple nonparametric curves with
# the shape-related nonlinear variation.  ------------------------------------------------------------------------------ Keywords:
# nonparametric smoothing, parametric modeling, common trend, mortality, Lee-Carter method, multi-populations
# ------------------------------------------------------------------------------ See also: twopop.R, data.R, optimization.R,
...
## descriptive plot plot kt of 36 countries including China
...
#### initial setting nonparametric smoothing 36 countries including China
for (i in 1:(loop.all - 1)) {
    kt = eval(parse(text = paste("kt.", names.all[i], ".female", sep = "")))
    t = eval(parse(text = paste(names.all[i])))$year
    d = data.frame(kt, t)
    
    # way 1 - locpol: locpol sm = locpol (kt~t, d, kernel = EpaK, xeval = t, bw = bw.default) # smooth kt nam6 = paste ('sm.kt',
    # names.all[i], 'female', sep = '.') temp6 = ts (sm$lpFit[,2], start = t[1], frequency = 1) assign (nam6, temp6)
    
    # way 2 - locpol with Cross Validation bandwidth nam7 = paste ('cvBwSel', names.all[i], 'female', sep = '.') temp7 =
    # regCVBwSelC(t,kt, deg = 1, kernel = EpaK, interval = c(0, 10)) assign (nam7, temp7) sm = locpol (kt~t, d, kernel = EpaK, xeval =
    # t, bw = temp7) # smooth kt nam6 = paste ('sm.kt', names.all[i], 'female', sep = '.') temp6 = ts (sm$lpFit[,2], start = t[1],
    # frequency = 1) assign (nam6, temp6)
    ...
    # way 4 - KernSmooth: locpoly sm = locpoly (t, kt, kernel = EpaK, bandwidth = bw.default, gridsize = length(t), range.x =
    # range(t)) nam6 = paste ('sm.kt', names.all[i], 'female', sep = '.') temp6 = ts (sm$y, start = t[1], frequency = 1) assign (nam6,
    # temp6)
    
    # way 5 - stats: ksmooth sm = ksmooth (t, kt, kernel = 'normal', bandwidth = bw.default, n.points = length(t), range.x = range(t))
    # nam6 = paste ('sm.kt', names.all[i], 'female', sep = '.') temp6 = ts (sm$y, start = t[1], frequency = 1) assign (nam6, temp6)
...
# way 1 - locpol: locpol sm = locpol (kt.China.female~years.mort, d, kernel = EpaK, xeval = years.mort, bw = bw.default)
# sm.kt.China.female = ts (sm$lpFit[,2], start = 1994, frequency = 1)

# way 2 - locpol with Cross Validation bandwidth cvBwSel.China.female = regCVBwSelC(years.mort,kt.China.female, deg = 1, kernel =
# EpaK, interval = c(0, 10)) sm = locpol (kt.China.female~years.mort, d, kernel = EpaK, xeval = years.mort, bw =
# cvBwSel.China.female) sm.kt.China.female = ts (sm$lpFit[,2], start = 1994, frequency = 1)
...
# way 4 - KernSmooth: locpoly sm = locpoly (years.mort, kt.China.female, kernel = EpaK, bandwidth = bw.default, gridsize =
# length(years.mort), range.x = range(years.mort)) sm.kt.China.female = ts (sm$y, start = 1994, frequency = 1)

# way 5 - stats: ksmooth sm = ksmooth (years.mort, kt.China.female, kernel = 'normal', bandwidth = bw.default, n.points =
# length(years.mort), range.x = range(years.mort)) sm.kt.China.female = ts (sm$y, start = 1994, frequency = 1)
...
#### intial values of thetas remove Russia, Lithuavia, Latvia, Estonia and Belarus set up the initial reference curve based on 31
#### countries
...
names.31 = c("Australia", "Austria", "Bulgaria", "Canada", "Chile", "CzechRepublic", "Denmark", "Finland", "France", "Germany", "Hungary", 
    ...
    "Slovenia", "Spain", "Switzerland", "Taiwan", "UnitedKingdom", "USA", "Sweden", "China")
...
# reference0.temp.nonsmooth = ts (reference0.temp, start = Sweden$year[1], frequency = 1) t.reference0.temp =
# time(reference0.temp.nonsmooth) ## smooth the initial reference curve # way 3 - sm: sm.regression reference0.temp.smooth =
# sm.regression(t.reference0.temp, reference0.temp.nonsmooth, eval.points = t.reference0.temp, model = 'none', poly.index = 1,
# display='none') reference0 = ts (reference0.temp.smooth$estimate, start = t.reference0.temp[1], frequency = 1) # way 5 - stats:
# ksmooth # reference0.temp.smooth = ksmooth(t.reference0.temp, reference0.temp.nonsmooth, kernel = 'normal', bandwidth =
# bw.default, n.points = length(t.reference0.temp), range.x = range(t.reference0.temp)) # reference0 = ts
# (reference0.temp.smooth$y, start = t.reference0.temp[1], frequency = 1)
...
for (j in 1:iteration) {
    ...
    # way 5 - stats: ksmooth temp17 = ksmooth(t.temp, reference.temp.nonsmooth, kernel = 'normal', bandwidth = bw.default, n.points =
    # length(t.temp), range.x = range(t.temp)) nam17 = paste ( 'reference', j, sep = '') assign( nam17, ts (temp17$y, start =
    # t.temp[1], frequency = 1))
