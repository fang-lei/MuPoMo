...
# Reference time series curve (unchanged). We use for comparing with resampled China's mortality TS
kt.referencet = reference4

# Original China's mortality TS, from which we are going to take TS bootstrap
...
loss = function(theta, t, kt, t.reference, kt.reference) {
    ...
    if (length(i0) > 0) {
        ...
        # way 3 - sm: sm.regression with optimal smoothing parameter h.optimal3 = h.select(sm.t, kt.reference) sm = sm.regression(sm.t,
        # kt.reference, h = h.optimal3, eval.points = t0, model = 'none', poly.index = 1, display='none') mu = theta1 * sm$estimate
...
hist(theta.boot[, 2][which(theta.boot[, 2] >= -100 & theta.boot[, 2] <= 100)], xlab = "theta 2", main = "Histogram of theta 2", xlim = c(-5, 
    5), breaks = 10)
quantile(theta.boot[, 2])

hist(theta.boot[, 3][which(theta.boot[, 3] >= 0.8 & theta.boot[, 3] <= 1.2)], xlab = "theta 3", main = "Histogram of theta 3", xlim = c(0.93, 
    0.99), breaks = 10)
...
# par(new = TRUE) plot(forecast(bootdata.fit, h = 30)$mean, main = '', axes = FALSE,xlab = '', ylab = '', xlim = c(1828, 2050),
# ylim = c(-200, 150))
...
# way 2 plot 95% confidence interval
...
for (i in 1:sim) {
    if (quan.ts.2010[i] >= quantile(quan.ts.2010, probs = c(0.45, 0.55))[1] & quan.ts.2010[i] <= quantile(quan.ts.2010, probs = c(0.45, 
        0.55))[2]) 
        quan.index.10[i] = i
}
...
for (i in 1:sim) {
    if (quan.ts.2010[i] >= quantile(quan.ts.2010, probs = c(0.1, 0.9))[1] & quan.ts.2010[i] <= quantile(quan.ts.2010, probs = c(0.1, 
        0.9))[2]) 
        quan.index.80[i] = i
}
...
for (i in 1:sim) {
    if (quan.ts.2010[i] >= quantile(quan.ts.2010, probs = c(0.05, 0.95))[1] & quan.ts.2010[i] <= quantile(quan.ts.2010, probs = c(0.05, 
        0.95))[2]) 
        quan.index.90[i] = i
}
